...
function GradualBlur(props) {
  ...
  // If target is page and animated==='scroll', recompute visibility on scroll
  useEffect(() => {
    if (config.target !== 'page' || config.animated !== 'scroll') return;
    const onScroll = () => {
      // trigger rerender by toggling visibility via intersection hook is not used for page; force style updates
      // No-op; style depends on window, but we can set state via a dummy setter if needed
    };
